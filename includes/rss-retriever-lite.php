...
const DO_COMMUNITY_RSS_ACCOUNTS               = 'rssrtvr_accaunts';
	...
	if ( stream_is_local( $url ) ) {
		...
		$response = wp_remote_get(
			...
			[
				...
				'timeout'     => 15,
				'redirection' => DO_COMMUNITY_RSS_MAX_CURL_REDIRECTS,
			]
		);
...
function rss_retrieval_get_headers( $url ) {
	...
	$args = [
		'timeout'     => 10,
		'redirection' => defined( 'DO_COMMUNITY_RSS_MAX_CURL_REDIRECTS' ) ? DO_COMMUNITY_RSS_MAX_CURL_REDIRECTS : 5,
	];
...
function rss_retrieval_default_options() {
	$defaults = [
		DO_COMMUNITY_RSS_ACCOUNTS             => [],
		DO_COMMUNITY_RSS_CHECK_DATE           => 0,
		DO_COMMUNITY_RSS_POST_LIFE_CHECK_DATE => 0,
		DO_COMMUNITY_RSS_MAX_EXEC_TIME        => 60,
		DO_COMMUNITY_RSS_SYNDICATED_FEEDS     => [],
		DO_COMMUNITY_RSS_FEEDS_UPDATED        => [],
		DO_COMMUNITY_RSS_PULL_MODE            => 'auto',
		DO_COMMUNITY_RSS_PC_INTERVAL          => 2 * DO_COMMUNITY_RSS_MIN_UPDATE_TIME,
		DO_COMMUNITY_RSS_FEED_PULL_TIME       => 0,
		DO_COMMUNITY_RSS_KEEP_IMAGES          => 'on',
	];
	...
	$options = get_option( DO_COMMUNITY_RSS_ACCOUNTS );
	...
	update_option( DO_COMMUNITY_RSS_ACCOUNTS, $options );
}
...
function rss_retrieval_save_image( $image_url, $preferred_name = '', $width = - 1, $height = - 1, $compression = - 1, $output_image_type = null ) {
	...
	if ( ! isset( $image_file ) || $image_file === false ) {
		...
		if ( $image_file === false ) {
			$image_file = rss_retrieval_file_get_contents( $image_url, false, false, false, DO_COMMUNITY_RSS_CURL_USER_AGENT );
		}
	}
...
function rss_retrieval_pack_content( $post, $extended = false ) {
	$packed_content = trim( $post['post_title'] ) . "\n\n" . DO_COMMUNITY_RSS_BLOCK_DIVIDER . "\n\n" . trim( $post['post_content'] ) . "\n\n" . DO_COMMUNITY_RSS_BLOCK_DIVIDER;
...
function rss_retrieval_unpack_content( $post, $packed_content ) {
	$parts = explode( DO_COMMUNITY_RSS_BLOCK_DIVIDER, $packed_content );
...
function rss_retrieval_get_youtube_video( $keyword ) {
	...
	$res = rss_retrieval_file_get_contents( 'https://www.youtube.com/results?search_query=' . urlencode( $keyword ) . '&sp=EgIYAw%253D%253D', false, '', 'self', DO_COMMUNITY_RSS_CURL_USER_AGENT );
...
function rss_retrieval_options_menu() {
	if ( isset( $_POST['submit_options'] ) && check_admin_referer( 'rss_retrieval_general_settings' ) ) {
		...
		if ( intval( $_POST[ DO_COMMUNITY_RSS_PC_INTERVAL ] ?? 0 ) * 60 >= get_option( DO_COMMUNITY_RSS_MAX_EXEC_TIME ) ) {
			$pseudo_cron_interval = intval( $_POST[ DO_COMMUNITY_RSS_PC_INTERVAL ] );
		} else {
			$pseudo_cron_interval = max( 2 * DO_COMMUNITY_RSS_MIN_UPDATE_TIME, round( get_option( DO_COMMUNITY_RSS_MAX_EXEC_TIME ) / 60 ) );
		}

		if ( update_option( DO_COMMUNITY_RSS_PULL_MODE, rss_retrieval_sanitize_user_input( wp_unslash( $_POST[ DO_COMMUNITY_RSS_PULL_MODE ] ) ) ) ) {
			wp_clear_scheduled_hook( 'rss_retrieval_update_by_wp_cron' );
		}

		if ( update_option( DO_COMMUNITY_RSS_PC_INTERVAL, $pseudo_cron_interval ) ) {
			wp_clear_scheduled_hook( 'rss_retrieval_update_by_wp_cron' );
		}

		update_option( DO_COMMUNITY_RSS_KEEP_IMAGES, isset( $_POST[ DO_COMMUNITY_RSS_KEEP_IMAGES ] ) ? 'on' : '' );
                    ...
                    <table class="form-table">
                        <tr>
                            ...
                            <td>
                                <select
                                        ...
                                        name="<?php echo esc_attr( DO_COMMUNITY_RSS_PULL_MODE ); ?>">
									<?php
									echo '<option ' . ( ( get_option( DO_COMMUNITY_RSS_PULL_MODE ) === 'auto' ) ? 'selected ' : '' ) . 'value="auto">Auto</option>';
									echo '<option ' . ( ( get_option( DO_COMMUNITY_RSS_PULL_MODE ) === 'cron' ) ? 'selected ' : '' ) . 'value="cron">Cron job or manually</option>';
									?>
                                </select>
                                <p id="auto" class="description" style="display:none;">
                                    In this mode, the RSS Retriever Lite plugin uses WordPress pseudo cron, which will
                                    be executed by the WordPress every
                                    <input type="number"
                                           min="<?php echo esc_attr( DO_COMMUNITY_RSS_MIN_UPDATE_TIME ); ?>"
                                           size="4"
                                           name="<?php echo esc_attr( DO_COMMUNITY_RSS_PC_INTERVAL ); ?>"
                                           value="<?php echo esc_attr( get_option( DO_COMMUNITY_RSS_PC_INTERVAL ) ); ?>">
                                    minutes.<br>
                                    The pseudo cron will trigger when someone visits your WordPress site, if the
                                    scheduled time has passed.
                                </p>
                                <p id="cron" class="description" style="display:none;">
                                    In this mode, you need to manually configure cron at your host. For example, if you
                                    want to run a cron job once a hour, just add the following line into your
                                    crontab:<br>
                                    <code><?php echo esc_html( '0 * * * * /usr/bin/curl --silent ' . esc_html( get_option( 'siteurl' ) ) . '/?pull-feeds=' . esc_attr( get_option( DO_COMMUNITY_RSS_CRON_MAGIC ) ) ); ?></code>
									<?php
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <p class="description">Maximum PHP execution time, given to RSS Retriever Lite to
...
function rss_retrieval_accounts_menu() {
	$accounts = get_option( DO_COMMUNITY_RSS_ACCOUNTS );

	if ( isset( $_POST['modify_accounts'] ) && check_admin_referer( 'DO_COMMUNITY_RSS_Accounts' ) ) {
		...
		update_option( DO_COMMUNITY_RSS_ACCOUNTS, $accounts );
	}
	?>
    <div class="wrap">
        ...
        <form method="post" action="<?php echo esc_url( rss_retrieval_request_uri() ); ?>">
			...
			<?php wp_nonce_field( 'DO_COMMUNITY_RSS_Accounts' ); ?>
            <br>
            <div style="text-align:left;">
...
function rss_retrieval_set_feed_options( $options ) {
	...
	if ( ! empty( $result['interval'] ) ) {
		$result['interval'] = max( DO_COMMUNITY_RSS_MIN_UPDATE_TIME, intval( $result['interval'] ) );
	}
    ...
    <div class="wrap">
		<?php if ( isset( $_POST['modify_selected_feeds'] ) ) { ?>
            ...
            <table class="widefat" style="margin: 8pt 0 8pt 0;">
                <tr>
                    <td>
                        <p>&#x1f4a1; Use the red check box to the left of each feed option you want to apply it to all
                            selected feeds. Unchecked options will not be applied.</p>
                    </td>
                </tr>
            </table>
			<?php
		...
		if (
			...
			update_option( DO_COMMUNITY_RSS_SYNDICATED_FEEDS, $rssrtvr_lite->feeds );
			...
			if ( $interval ) {
				$interval = max( DO_COMMUNITY_RSS_MIN_UPDATE_TIME, $interval );
			}
			...
			update_option( DO_COMMUNITY_RSS_FEED_OPTIONS, $rssrtvr_lite->global_options );
			$rssrtvr_lite->showMainPage( false );
...
class class_rss_retrieval_syndicator {
	public $langs;
	public $post = [];
	public $link_checked;
	public $insideitem;
	...
	function __construct() {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
		$option_value = $wpdb->get_var( $wpdb->prepare( "SELECT option_value FROM {$wpdb->prefix}options WHERE option_name = %s LIMIT 1", DO_COMMUNITY_RSS_FEEDS_UPDATED ) );
		if ( $option_value ) {
			$this->feeds_updated = maybe_unserialize( $option_value );
		}

		$this->blog_charset   = strtoupper( get_option( 'blog_charset' ) );
		$this->global_options = $this->init_feed_options( get_option( DO_COMMUNITY_RSS_FEED_OPTIONS ) );
		update_option( DO_COMMUNITY_RSS_FEED_OPTIONS, $this->global_options );

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
		$option_value = $wpdb->get_var( $wpdb->prepare( "SELECT option_value FROM {$wpdb->prefix}options WHERE option_name = %s LIMIT 1", DO_COMMUNITY_RSS_SYNDICATED_FEEDS ) );
		if ( $option_value ) {
	...
	function add_custom_cron_interval( $schedules ) {
		$schedules[ DO_COMMUNITY_RSS_PC_NAME ] = [
			'interval' => intval( get_option( DO_COMMUNITY_RSS_PC_INTERVAL ) ) * 60,
			'display'  => esc_html( DO_COMMUNITY_RSS_PC_NAME ),
		];
	...
	function update_feeds() {
		if ( time() > get_option( DO_COMMUNITY_RSS_FEED_PULL_TIME ) + get_option( DO_COMMUNITY_RSS_MAX_EXEC_TIME ) + 30 ) {
			wp_cache_flush();
	...
	function log( $message ) {
		if ( ! $this->preview ) {
			...
			update_option( DO_COMMUNITY_RSS_LOG, $this->parse_feed_log );
		}
	}
	...
	private function maybe_increase_time_limit() {
		$max_exec_time = (int) get_option( 'DO_COMMUNITY_RSS_MAX_EXEC_TIME' );
			...
			for ( $i = 0; $i < $feeds_cnt; $i ++ ) {
				if ( in_array( $i, $feed_ids ) ) {
					if ( ! $check_time || $this->getUpdateTime( $i ) === 'asap' ) {
						update_option( DO_COMMUNITY_RSS_FEED_PULL_TIME, time() );
	...
	function insertPost() {
		...
		if ( strpos( $this->post['link'], '.youtube.com/' ) !== false || strpos( $this->post['link'], '//youtube.com/' ) !== false ) {
			...
			$content = rss_retrieval_file_get_contents( $this->post['link'], false, '', 'self', DO_COMMUNITY_RSS_CURL_USER_AGENT );
		...
		if ( $this->current_feed['options']['translator'] !== 'none' ) {
			...
			if ( $this->current_feed['options']['translator'] === 'yandex_translate' ) {
				$translated = rss_retrieval_yandex_translate( get_option( DO_COMMUNITY_RSS_ACCOUNTS )['yandex_api_key'], $packed_content, $this->current_feed['options']['yandex_translation_dir'] );
			} elseif ( $this->current_feed['options']['translator'] === 'google_translate' ) {
				$translated = rss_retrieval_google_translate( get_option( DO_COMMUNITY_RSS_ACCOUNTS )['google_api_key'], $packed_content, $this->current_feed['options']['google_translation_source'], $this->current_feed['options']['google_translation_target'] );
			} elseif ( $this->current_feed['options']['translator'] === 'deepl_translate' ) {
				$translated = rss_retrieval_deepl_translate( get_option( DO_COMMUNITY_RSS_ACCOUNTS )['deepl_api_key'], $packed_content, $this->current_feed['options']['deepl_translation_target'], ( $this->current_feed['options']['deepl_use_api_free'] === 'on' ) );
			}
		...
		if ( ! add_post_meta( $post_id, '_rssrtvr_post_name', $DO_COMMUNITY_RSS_Post_name ) ) {
		...
		if ( get_post_meta( $post_id, '_rssrtvr_rss_source', true ) === '' ) {
			$this->log( 'Unable to set the DO_COMMUNITY_RSS_source filed' );
			$this->log( 'The post will not be added' . PHP_EOL );
			$this->delete_post_media( $post_id );
			wp_delete_post( $post_id, true );
	...
	function showSettings( $islocal, $settings, $change_selected = false ) {
        ...
        <form name="feed_settings"
              action="<?php echo esc_url( preg_replace( '/\&edit-feed-id\=[0-9]+/', '', rss_retrieval_request_uri() ) ); ?>"
              method="post">
				...
				">
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="post_author">
								<?php
								$wp_user_search = get_users( array(
									'role__in' => [
										'author',
										'editor',
										'administrator'
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            <p class="description">The period of time after which the post will be deleted. If you don't
                                want to limit the post lifetime, set this parameter to 0.</p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            minutes.
                            <p class="description">This range will be used to randomly adjust the publication date for
                                every generated post. For example, if you set the adjustment range as
                                <code>[0..60]</code>,
                                the post dates will be increased by a random value between 0 and 60 minutes. Negative
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description">This option defines what the RSS Retriever Lite syndicator have to do
                                if none of the post categories mutch the predefined defined ones [<a
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            <label for="create_tags">when checked, post category names will be added as post
                                tags.</label>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            <label for="auto_tags">if checked, the RSS Retriever Lite plugin will look for existing tags
                                within your content and add them automatically.</label>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            <label for="tags_to_woocommerce">check this option to assign importing post tags to
                                WooCommerce product tags.</label>
                        </td>
                    </tr>
				...
				">
                    <tr>
                        ...
                        <td>
							...
							?>
                            <label for="store_images">if checked, all images will be copied into the default uploads
                                folder [<a href="https://www.rssretriever.com/documentation/#store-images-locally"
                                           target="_blank">?</a>]</label>
                        </td>
                    </tr>

                    <th scope="row"
                        id="image_format_selector"><?php $this->showChangeBox( $change_selected, 'image_format' ); ?>
                    ...
                    <td>
                        ...
                        <input style="vertical-align: middle;" type="number" id="compression_quality"
                               name="compression_quality" min="10" max="100"
                               value="<?php echo esc_attr( $settings['compression_quality'] ); ?>" size="4">
                        <p class="description">Select the format to which you want to convert locally stored PNG
                            images.</p>
                    </td>
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            <label for="add_to_media_library">if checked, all images will be added to the WordPress
                                Media Library [<a
                                        href="https://www.rssretriever.com/documentation/#add-to-media-library"
                                        target="_blank">?</a>]</label>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description">If enabled the RSS Retriever Lite syndicator inserts media
                                attachments (if available) into the aggregating post. The
                                following types of attachments are supported: <code>&lt;media:content&gt;</code>, <code>&lt;media:thumbnail&gt;</code>
                                and <code>&lt;enclosure&gt;</code>.
                                <br>All the aggregated images will contain <code>class="media_thumbnail"</code> in the
                                <code>&lt;img&gt;</code> tag.
                            </p>
                        </td>
                    </tr>
				...
				">
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            <p class="description">The alternative post thumbnail source URL for the case if the source
                                image was not found.</p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            <label for="require_thumbnail">if the plugin will not be able to create post thumbnail as
                                specified above (e.g. the source image is missing or broken), the post will not be
                                published.</label>
                        </td>
                    </tr>
				...
				">
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            <p class="description">Post slug template. By default it's the same as title, but you can
                                alter it according to your needs.</p>
                        </td>
                    </tr>
                        ...
                        <td>
                            <div style="padding: 1em; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;">
                                <ul style="margin-top: 0.5em;">
                                    <li><code>%link%</code>
                                        – <?php echo esc_html__( 'Post link (URL)', 'rss-retriever-lite' ); ?></li>
                                    <li><code>%post_title%</code>
                                        – <?php echo esc_html__( 'Post title', 'rss-retriever-lite' ); ?></li>
                                    <li><code>%post_content%</code>
                                        – <?php echo esc_html__( 'Post content (HTML)', 'rss-retriever-lite' ); ?></li>
                                    ...
                                    <li><code>%post_excerpt%</code>
                                        – <?php echo esc_html__( 'Post excerpt (HTML)', 'rss-retriever-lite' ); ?></li>
                                    ...
                                    <li><code>%enclosure_url%</code>
                                        – <?php echo esc_html__( 'Media enclosure URL', 'rss-retriever-lite' ); ?></li>
                                    <li><code>%media_description%</code>
                                        – <?php echo esc_html__( 'Media description', 'rss-retriever-lite' ); ?></li>
                                    ...
                                    <li><code>%post_guid%</code>
                                        – <?php echo esc_html__( 'Post GUID', 'rss-retriever-lite' ); ?></li>
                            ...
                            <p class="description">
                                <strong>Usage tips:</strong> Placeholders can be used in post title, slug, content, and
                                excerpt templates. For indexed placeholders, use a number inside brackets (e.g. <code>%media_thumbnail[0]</code>).
                                For custom date formats, use PHP date format strings (e.g.
                                <code>%post_date[Y-m-d]</code>). For XML tags and attributes, use the tag name and
                                attribute name (e.g. <code>%xml_tags[author]</code>, <code>%xml_tags_attr[media:content][url]</code>)
				...
				">
                    <tr>
                        ...
                        <td>
							...
							?>
                            <p class="description">Enter a comma-separated list of tags to remove from the generated
                                posts, e.g.: <code>a, h1, img</code>.</p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            <label for="utf8_encoding">enables UTF-8 encoding. This option converts an ISO-8859-1 string
                                to UTF-8 that may be required when parsing the XML/RSS feeds containing invalid UTF-8
                                start bytes e.g. <code>
                                    <0x92>
                                </code>.</label>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
							...
							?>
                            <label for="convert_encoding">enables character encoding conversion. This option may be
                                useful for parsing XML/RSS feeds in national character sets other than UTF-8.</label>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description">
                                ...
                                <a href="https://www.rssretriever.com/documentation/#custom-fields"
                                   target="_blank">[?]</a>
                            </p>
                            ...
                            <p class="description">
                                ...
                                The tag name on the left must match the XML source. The field name on the right must be
                                an existing WordPress custom field (meta key).
                            </p>

                            <p class="description">
                                Example:<br>
                                <code>title-&gt;my_custom_title</code><br>
                                This rule saves the value of the <code>&lt;title&gt;</code> XML tag into the <code>my_custom_title</code>
                                custom field.<br>
                                If you are using plugins like WooCommerce, their meta keys often start with an
                                underscore (e.g. <code>_price</code>).
                            </p>
            ...
            <div id="filtering" class="tab_content">
                ...
                <br>
                <p class="description">The filtering routine is case-insensitive and it does not match full words. Thus
                    please be careful when using short keywords that may unintentionally match parts of other words.</p>
            </div>
			<?php
	...
	function showMainPage( $showsettings = true ) {
        ...
        <div class="metabox-holder postbox-container" style="width:100%;">
            <form style="padding: 22px 12px 22px 12px; margin-bottom: 24px;"
                  action="<?php echo esc_url( rss_retrieval_request_uri() ); ?>" method="post">
	...
	function enqueue_scripts( $hook ) {
		if (
			...
			$hook !== 'rss-retriever-lite_page_DO_COMMUNITY_RSS_Accounts' &&
		...
		wp_localize_script(
			...
			'rss_retrieval_vars',
			[
				'rss_pull_mode' => get_option( DO_COMMUNITY_RSS_PULL_MODE ),
				'post_type_map' => $map,
			]
		);
	}
	...
	function cron_init() {
		if ( get_option( DO_COMMUNITY_RSS_PULL_MODE ) === 'auto' ) {
			add_action( 'rss_retrieval_update_by_wp_cron', [ $this, 'update_feeds' ] );
			if ( ! wp_next_scheduled( 'rss_retrieval_update_by_wp_cron' ) ) {
				wp_schedule_event( time(), DO_COMMUNITY_RSS_PC_NAME, 'rss_retrieval_update_by_wp_cron' );
			}
...
if ( ! is_admin() ) {
	...
	if ( $pull_feeds === get_option( 'DO_COMMUNITY_RSS_CRON_MAGIC' ) ) {
		if ( ! function_exists( 'wp_insert_category' ) ) {
	...
	if ( ( time() - get_option( DO_COMMUNITY_RSS_POST_LIFE_CHECK_DATE ) ) > DO_COMMUNITY_RSS_POST_LIFE_CHECK_PERIOD ) {
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
		$post_ids = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT post_id 
		...
		update_option( DO_COMMUNITY_RSS_POST_LIFE_CHECK_DATE, time() );
	}
} else {
	...
	if ( get_option( DO_COMMUNITY_RSS_KEEP_IMAGES ) !== 'on' ) {
...
function rss_retrieval_main_menu() {
	...
	add_submenu_page(
		...
		esc_html__( 'Syndicator log', 'rss-retriever-lite' ),
		'manage_options',
		'class_rss_retrieval_syndicator_log',
